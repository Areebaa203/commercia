...
export default function OrdersPage() {
  ...
  const [stats, setStats] = useState([
    { label: "Total Orders", value: "0", change: "0%", trend: "up", icon: "mingcute:shopping-bag-3-fill", color: "bg-blue-500" },
    { label: "Pending", value: "0", change: "0%", trend: "down", icon: "mingcute:time-fill", color: "bg-yellow-500" },
    { label: "Completed", value: "0", change: "0%", trend: "up", icon: "mingcute:check-circle-fill", color: "bg-green-500" },
    { label: "Refunded", value: "0", change: "0%", trend: "up", icon: "mingcute:refresh-2-fill", color: "bg-red-500" },
  ]);

  const [selectedStatus, setSelectedStatus] = useState("All");
  const [searchQuery, setSearchQuery] = useState("");
  const [dropdownFilters, setDropdownFilters] = useState({ status: "All", payment: "All" });
  ...
  const fetchOrders = useCallback(async (page = 1, filter = "All", q = "", dFilters = {}, dRange = {}) => {
    try {
      setLoading(true);
      let url = `/api/dashboard/orders?page=${page}&pageSize=${pageSize}&filter=${filter}&q=${encodeURIComponent(q)}`;
      
      if (dFilters.status && dFilters.status !== "All") url += `&status=${dFilters.status}`;
      if (dFilters.payment && dFilters.payment !== "All") url += `&payment=${dFilters.payment}`;
      ...
      if (json.success) {
        const mapped = json.data.map(o => ({
          ...
          customer: {
            name:
              (o.customer?.name && String(o.customer.name).trim()) ||
              (o.checkout_details?.shippingName && String(o.checkout_details.shippingName).trim()) ||
              ((o.customer?.email || o.checkout_details?.contactEmail) ? String(o.customer?.email || o.checkout_details?.contactEmail).split("@")[0] : "") ||
              "Guest",
            email:
              (o.customer?.email && String(o.customer.email).trim()) ||
              (o.checkout_details?.contactEmail && String(o.checkout_details.contactEmail).trim()) ||
              "N/A",
            avatar:
              o.customer?.avatar_url ||
              `https://api.dicebear.com/9.x/avataaars/svg?seed=${o.order_number}`
          },
        ...
        const s = json.stats;
        setStats([
          { label: "Total Orders", value: s.total.toLocaleString(), change: "+0%", trend: "up", icon: "mingcute:shopping-bag-3-fill", color: "bg-blue-500" },
          { label: "Pending", value: s.pending.toLocaleString(), change: "0%", trend: "down", icon: "mingcute:time-fill", color: "bg-yellow-500" },
          { label: "Completed", value: s.completed.toLocaleString(), change: "0%", trend: "up", icon: "mingcute:check-circle-fill", color: "bg-green-500" },
          { label: "Refunded", value: s.refunded.toLocaleString(), change: "0%", trend: "up", icon: "mingcute:refresh-2-fill", color: "bg-red-500" },
        ]);
      }
    } catch (err) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
        fetchOrders(currentPage, selectedStatus, searchQuery, dropdownFilters, dateRange);
    }, 300);
            ...
            <div className="flex items-center gap-2 overflow-x-auto max-w-full pb-2 sm:pb-0 no-scrollbar">
                ...
                <div className="flex items-center gap-1">
                    {Array.from({ length: Math.ceil(totalResults / pageSize) }).map((_, i) => {
                        const page = i + 1;
                        // Basic pagination logic to show only few pages if there are many
                        if (
                            page === 1 || 
