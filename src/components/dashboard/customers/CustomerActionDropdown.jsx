...
const CustomerActionDropdown = ({ isOpen, onClose, anchorRef, customer, onView, onEdit, onDelete }) => {
  ...
  return (
    ...
    >
      <div className="p-1">
        <button 
            onClick={() => { onClose(); onView && onView(customer); }}
        ...
        <button 
            onClick={() => { onClose(); onEdit && onEdit(customer); }}
            className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900 transition-colors text-left"
        >
        ...
        <button 
            onClick={() => { onClose(); onDelete && onDelete(customer); }}
