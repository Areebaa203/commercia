...
import { parseProductImages, stringifyProductImages } from "@/lib/product-images";
const EditProductModal = ({ isOpen, onClose, product, onSuccess }) => {
    ...
    if (isOpen) {
      ...
      if (product) {
        reset({
          ...
          image_url: stringifyProductImages(parseProductImages(product.image_url || "")),
        });
      }
    } else {
  ...
  const onSubmit = async (data) => {
    setLoading(true);
    try {
      await axios.patch(`/api/products/update/${product.id}`, data);
      ...
      setErrorDialog({ open: true, message: "Failed to update product: " + msg });
    } finally {
      setLoading(false);
    }
  };
