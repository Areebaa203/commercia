          ...
          <div className="space-y-6">
            <div className="aspect-video w-full overflow-hidden rounded-xl bg-gray-100 border border-gray-200">
              {String(content.type || "").toLowerCase() === "video" ? (
                <video
                  src={content.thumbnail_url || content.thumbnail}
                  controls
            ...
            <div className="space-y-4">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className={clsx("inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium",
                    content.status?.toLowerCase() === "published" ? "bg-green-100 text-green-700" :
                      content.status?.toLowerCase() === "draft" ? "bg-gray-100 text-gray-700" :
                        "bg-blue-100 text-blue-700"
                  )}>
                    {formatStatus(content.status)}
                  </span>
                  <span className="text-xs text-gray-500">•</span>
                  <span className="text-xs text-gray-500">{content.typeLabel || content.type}</span>
                  <span className="text-xs text-gray-500">•</span>
              ...
              <div className="flex items-center gap-3 p-3 rounded-lg border border-gray-100 bg-gray-50">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">{content.author_name || content.author}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div className="p-3 rounded-lg border border-gray-100 bg-white text-center">
                  ...
                  <span className="block text-lg font-bold text-gray-900">{content.comments_count ?? content.comments}</span>
