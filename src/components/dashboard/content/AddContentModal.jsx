...
const AddContentModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const onSubmit = async (values) => {
    ...
    setLoading(true);
    try {
      await axios.post("/api/content/add", payload);
      onSuccess?.();
      onClose();
    } catch (error) {
      const msg =
      ...
      console.error("Error creating content:", msg);
      setErrorDialog({ open: true, message: "Failed to create content: " + msg });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-gray-100 bg-white px-6 py-4">
          <h3 className="text-lg font-bold text-gray-900">Create New Content</h3>
          <button
            type="button"
            onClick={onClose}
